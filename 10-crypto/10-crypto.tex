...
The file we are interested in is the source code for the OpenSSL library,
which has a name similar to \texttt{openssl-3.3.2.tar.gz}
(by the time of this tutorial, there will probably be a newer version)
but \textbf{don't}
...
Next to the file name are \NAME{Checksums}, at time of writing SHA256, PGP,
and SHA1. You \textbf{should} download the two SHA signatures, each of which
is a very short text file.

Once you have your own copy of \texttt{openssl-3.3.2.tar.gz} (or the current
version for the tutorial), run the hash
...
as a CDN or local archive, can you still use the signature from the OpenSSL
web site? Why?
...
Have a quick look at the source code. This program is similar to the client
\texttt{TCP Client Server}
